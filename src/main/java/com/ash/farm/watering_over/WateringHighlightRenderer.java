...
public class WateringHighlightRenderer {
    ...
    public static void renderWateringHighlightBox(PoseStack poseStack, BlockPos blockPos, ResourceLocation texture, EnumMap<Direction, Boolean> connectedSides) {
        ...
        RenderType type = RenderType.beaconBeam(texture, true);
        VertexConsumer buffer = Minecraft.getInstance().renderBuffers().bufferSource().getBuffer(type);
        Camera camera = Minecraft.getInstance().gameRenderer.getMainCamera();
//        Quaternionf quaternionf = new Quaternionf();
//        quaternionf.rotateX((float) Math.toRadians(camera.getXRot()));
...
//        poseStack.mulPose(quaternionf);
    ...
    private static void renderHighlightCubeSide(PoseStack poseStack, VertexConsumer buffer, Direction direction, AABB renderedCubeAABB, Vec3 from, Vec3 to) {
        ...
        int alpha = 100;
        int light = 0x0000ff;
//        LevelRenderer.renderFace(poseStack, buffer, direction, (float) from.x, (float) from.y, (float) from.z, (float) to.x, (float) to.y, (float) to.z, 1.0f, 1.0f, 1.0f, 0.2f);
        if (normalOrdinal == -1)
