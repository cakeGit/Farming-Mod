...
public class WateringControls {
    ...
    public static final Lazy<KeyMapping> TOGGLE_MODE = Lazy.of(() -> new KeyMapping(
        "key." + AshFarm.MODID + ".toggle_mode",
        KeyConflictContext.IN_GAME,
        InputConstants.Type.KEYSYM,
        GLFW.GLFW_KEY_B,
        "key.categories." + AshFarm.MODID
    ));
    ...
    public static void tickControls() {
        ...
        while (TOGGLE_MODE.get().consumeClick()) {
            ...
            Minecraft.getInstance().player.displayClientMessage(Component.translatable("chat." + AshFarm.MODID + ".option_title").withStyle(ChatFormatting.GRAY).append(Component.literal("[").withStyle(ChatFormatting.WHITE, ChatFormatting.BOLD)).append(Component.literal(newSelector.name()).withStyle(newSelector.getChatFormatting(), ChatFormatting.BOLD)).append(Component.literal("]").withStyle(ChatFormatting.WHITE, ChatFormatting.BOLD)), true);
        }
    }
    
    @EventBusSubscriber(modid = AshFarm.MODID, bus = EventBusSubscriber.Bus.MOD, value = Dist.CLIENT)
